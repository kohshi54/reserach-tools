...
echo "ooo/all_pkts for each flow and all"
if [ "$2" == debug ]; then 
	echo "c_ip,c_pkts_all,c_pkts_ooo,s_ip,s_pkts_all,s_pkts_ooo"
	echo "-------"
fi
...
#	print "s_pkts_ooo: " s_pkts_ooo
	...
	total_c_pkts_ooo += c_pkts_ooo
	total_s_pkts_all += s_pkts_all
	total_s_pkts_ooo += s_pkts_ooo

	c_result = c_pkts_ooo / c_pkts_all
	s_result = s_pkts_ooo / s_pkts_all

	if (c_result != 0.00 || s_result != 0.00) {
		if (debug == "debug") {
			print $1,$3,$10,$12,$15,$17,$24,$26
...
END {
	...
	c_result = total_c_pkts_ooo / total_c_pkts_all
	s_result = total_s_pkts_ooo / total_s_pkts_all
